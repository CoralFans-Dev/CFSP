...
void ComandManager::registerGuiCommand() {
    ...
    this->command->runtimeOverload()
        ...
        .execute([this](CommandOrigin const& origin, CommandOutput& output, ll::command::RuntimeCommand const& self) {
            ...
            if (!cfsp.has_value()) return output.error("command.error.spNoFind"_tr());
            if (!manager::CFSPManager::getInstance().isManager(player.value())
                && !cfsp.value()->hasPermission(player.value(), simulated_player::SimPlayerPermission::None))
    ...
    this->command->runtimeOverload()
        .text("g")
        .optional("name", ll::command::ParamKind::SoftEnum, "cfspgroupname")
        .execute([this](CommandOrigin const& origin, CommandOutput& output, ll::command::RuntimeCommand const& self) {
            auto player = this->tryGetPlayer(origin);
            if (!player.has_value()) return output.error("command.fail.onlyplayer"_tr());
            ...
            if (!group.has_value()) return output.error("command.error.groupNoFind"_tr());
            if (!manager::CFSPManager::getInstance().isManager(player.value())
                && !group.value()->hasPermission(player.value(), group::GroupPermission::None))
                return output.error("command.error.permissionDenied"_tr());
            gui::GuiManager::getInstance().sendGroupOperatorList(*player.value(), group.value());
        });
}
} // namespace coral_fans::cfsp::command