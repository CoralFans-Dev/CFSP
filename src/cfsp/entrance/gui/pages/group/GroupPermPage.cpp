...
void GroupPermPage::sendTo(Player& player) {
    ...
    for (auto i : mGroup->mData.permission)
    ...
    if (size) {
        form.appendButton(splist[0].first, [this, self, targetPlayer = splist[0]](Player& player) {
            std::make_shared<GroupPermPage2>(mGroup, self, targetPlayer.first, targetPlayer.second)->sendTo(player);
        });
        for (int i = 1; i < size; i++)
            if (splist[i] != splist[i - 1])
                form.appendButton(splist[i].first, [this, self, targetPlayer = splist[i]](Player& player) {
                    std::make_shared<GroupPermPage2>(mGroup, self, targetPlayer.first, targetPlayer.second)
                        ->sendTo(player);
                });
    }
    form.appendButton("gui.perm.publicGroup"_tr(), [this, self](Player& player) {
        std::make_shared<GroupPermPage2>(mGroup, self)->sendTo(player);
    });
