...
void SpActionPage::sendTo(Player& player) {
    using ll::i18n_literals::operator""_tr;
    auto                     form = ll::form::CustomForm("gui.action.spTitle"_tr());
    std::vector<std::string> op;
    if (mPerm & (uint)simulated_player::SimPlayerPermission::Attack) op.emplace_back("gui.action.attack"_tr());
    if (mPerm & (uint)simulated_player::SimPlayerPermission::Build) op.emplace_back("gui.action.build"_tr());
    if (mPerm & (uint)simulated_player::SimPlayerPermission::Interact) op.emplace_back("gui.action.interact"_tr());
    if (mPerm & (uint)simulated_player::SimPlayerPermission::Jump) op.emplace_back("gui.action.jump"_tr());
         ...
         op](Player& player, ll::form::CustomFormResult const& elements, ll::form::FormCancelReason cancelReason) {
            ...
            if (eleOp.value() == "gui.action.attack"_tr())
                return manager::CFSPManager::getInstance()
                    .spAttack(&player, mCfsp->mSaveData.name, times.value(), interval.value())
                    .sendTo(player);
            if (eleOp.value() == "gui.action.build"_tr())
                return manager::CFSPManager::getInstance()
                    .spBuild(&player, mCfsp->mSaveData.name, times.value(), interval.value())
                    .sendTo(player);
            if (eleOp.value() == "gui.action.interact"_tr())
                return manager::CFSPManager::getInstance()
                    .spInteract(&player, mCfsp->mSaveData.name, times.value(), interval.value())
                    .sendTo(player);
            if (eleOp.value() == "gui.action.jump"_tr())
                return manager::CFSPManager::getInstance()
                    .spJump(&player, mCfsp->mSaveData.name, times.value(), interval.value())
                    .sendTo(player);
