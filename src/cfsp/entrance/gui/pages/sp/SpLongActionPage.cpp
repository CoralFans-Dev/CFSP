...
void SpLongActionPage::sendTo(Player& player) {
    using ll::i18n_literals::operator""_tr;
    auto                     form = ll::form::CustomForm("gui.longAction.spTitle"_tr());
    std::vector<std::string> op;
    if (mPerm & (uint)simulated_player::SimPlayerPermission::Use) op.emplace_back("gui.longAction.use"_tr());
    if (mPerm & (uint)simulated_player::SimPlayerPermission::Destroy) op.emplace_back("gui.longAction.destroy"_tr());
         ...
         op](Player& player, ll::form::CustomFormResult const& elements, ll::form::FormCancelReason cancelReason) {
            ...
            if (eleOp.value() == "gui.longAction.use"_tr())
                return manager::CFSPManager::getInstance()
                    .spUse(&player, mCfsp->mSaveData.name, _long.value(), times.value(), interval.value())
                    .sendTo(player);
            if (eleOp.value() == "gui.longAction.destroy"_tr())
                return manager::CFSPManager::getInstance()
                    .spDestroy(&player, mCfsp->mSaveData.name, _long.value(), times.value(), interval.value())
                    .sendTo(player);
