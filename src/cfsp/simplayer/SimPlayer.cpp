...
bool loadSpNbt(SimulatedPlayer* sp, std::filesystem::path basePath) {
    ...
    f.close();
    try {
        sp->load(CompoundTag::fromSnbt(snbt).value(), *mod().getDefaultDataLoadHelper());
    } catch (...) {
        return false;
    }
    return true;
...
SimPlayerManager::spawnSimPlayer(Player* player, std::string const& name, Vec3 const& pos, Vec2 const& rot) {
    ...
    if (rejoin) {
        ...
        sputils::loadSpNbt(simPlayer, CFSP::getInstance().getSelf().getDataDir() / "simplayer" / spIt->second.xuid);
...
SimPlayerManager::despawnSimPlayer(Player* player, std::string const& spname, bool noCheck) {
    ...
    if (player->getCommandPermissionLevel() >= mod().getConfig().simPlayer.adminPermission || noCheck
        ...
        if (it->second.simPlayer) {
            const auto& basePath = CFSP::getInstance().getSelf().getDataDir() / "simplayer" / it->second.xuid;
...
std::pair<std::string, bool> SimPlayerManager::rmSimPlayer(Player* player, std::string const& spname, bool noCheck) {
    ...
    if (player->getCommandPermissionLevel() >= mod().getConfig().simPlayer.adminPermission || noCheck
        ...
        // remove
        std::filesystem::remove_all(CFSP::getInstance().getSelf().getDataDir() / "simplayer" / it->second.xuid);
